...
async function main() {
  ...
  const absolutePath = path.isAbsolute(actualPath) ? actualPath : path.resolve(process.cwd(), actualPath);
  ...
  console.log("Size:", fs.existsSync(absolutePath) ? fs.statSync(absolutePath).size + " bytes" : "(missing)");
